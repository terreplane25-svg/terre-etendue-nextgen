...
export default function PillarCard({ title, subtitle, description, href, accent, stats }: PillarCardProps) {
  const accentColor = accent === 'gold' ? '[var(--gold)]' : '[var(--cyan)]';

  return (
    <Link href={href} className="block group">
      <div className="card card-hover p-7 space-y-4 h-full">
        <div className="flex items-center justify-between">
          <h3 className={`font-display text-xl font-bold text-[var(--text)] group-hover:text-${accentColor} transition-colors duration-300`}>
            {title}
          </h3>
        ...
        <p className="text-[var(--text)]/35 text-sm font-body leading-relaxed">
          {description}
        </p>
        {stats && (
          <div className="flex gap-6 pt-2">
            {stats.map((stat) => (
              <div key={stat.label}>
                ...
                <span className="text-[var(--text)]/20 text-xs ml-1.5 font-mono">{stat.label}</span>
              </div>
            ))}
          </div>
        )}
