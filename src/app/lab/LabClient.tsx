  ...
  loading: () => (
    <div className="w-full h-[500px] hud-panel flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-[10px] text-[#C8D8E8]/20" style={{fontFamily:'Share Tech Mono,monospace'}}>LOADING 3D...</p>
      </div>
    </div>
  ),
});
      ...
      <div className="max-w-[960px] mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[9px] tracking-[0.2em] text-[#00C8FF]/50 uppercase" style={{fontFamily:'Orbitron,sans-serif'}}>Pilier 04</span>
          </div>
          <h1 className="text-2xl md:text-3xl font-bold text-[#C8D8E8] mb-8" style={{fontFamily:'Orbitron,sans-serif'}}>LE LAB</h1>
        </motion.div>
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {sims.map((sim) => (
            <button key={sim.id} onClick={() => !sim.disabled && setActive(sim.id)} disabled={sim.disabled}
              className={`hud-btn text-[8px] px-4 py-2 ${active === sim.id ? '' : sim.disabled ? 'opacity-20 cursor-not-allowed' : 'opacity-40'}`}
        ...
        <motion.div key={active} initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
          ...
          {active !== "tawhid" && <div className="w-full h-[400px] hud-panel flex items-center justify-center"><p className="text-[#C8D8E8]/15 text-sm" style={{fontFamily:'Share Tech Mono,monospace'}}>EN DÉVELOPPEMENT</p></div>}
        </motion.div>
        <div className="mt-6 hud-panel p-5">
          <p className="hud-label text-[#C8D8E8]/25 mb-2">{sims.find(s => s.id === active)?.title}</p>
          <p className="text-sm text-[#C8D8E8]/30" style={{fontFamily:'Rajdhani,sans-serif'}}>{sims.find(s => s.id === active)?.desc}</p>
        </div>
      </div>
    </div>
  );
