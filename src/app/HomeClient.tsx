  ...
  return (
    <div>
      ...
      <motion.section initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.8 }}
        style={{ position: 'relative', overflow: 'hidden', height: 360, display: 'flex', alignItems: 'flex-end', marginBottom: 32 }}>
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(244,245,247,1) 0%, rgba(244,245,247,0.6) 50%, rgba(244,245,247,0.1) 100%)' }} />
        <div style={{ position: 'relative', maxWidth: 1200, margin: '0 auto', padding: '0 24px 36px', width: '100%' }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: dash.lavender, letterSpacing: '0.12em', textTransform: 'uppercase' as const, marginBottom: 8, fontFamily: dash.fontMono }}>Tableau de bord</div>
          <h1 style={{ fontSize: 36, fontWeight: 800, color: dash.ink, marginBottom: 6, lineHeight: 1.15 }}>Terre Étendue Islam</h1>
          <p style={{ fontSize: 15, color: dash.inkSoft, maxWidth: 480 }}>Revue indépendante de cosmologie · Examen critique · Données empiriques · Sources sacrées</p>
        </div>
      </motion.section>
            ...
            <motion.div key={p.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45, delay: 0.35 + i * 0.06 }}>
              <Link href={p.href} className="dash-card" style={{ display: 'flex', flexDirection: 'column', cursor: 'pointer', overflow: 'hidden' }}>
                <div style={{ height: 120, overflow: 'hidden', position: 'relative' }}>
                  ...
                  <span className="badge" style={{ position: 'absolute', top: 12, right: 12, background: 'rgba(255,255,255,0.9)', color: p.color, backdropFilter: 'blur(8px)' }}>{p.num}</span>
                </div>
          ...
          {recent.map((a, i) => (
            <motion.div key={a.slug} initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.35, delay: 0.5 + i * 0.04 }}>
              <Link href={`/article/${a.slug}`} className="dash-card" style={{ display: 'flex', overflow: 'hidden', cursor: 'pointer' }}>
                <div style={{ width: 160, minHeight: 110, flexShrink: 0, overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ padding: '16px 20px', flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11, color: dash.inkGhost, marginBottom: 6, fontFamily: dash.fontMono }}>
                    Terre Etendue · {a.readTime} min
                  </div>
                  {a.description && <div style={{ fontSize: 13, color: dash.inkMuted, lineHeight: 1.5, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{a.description}</div>}
                </div>
              </Link>
            </motion.div>
          ))}
