 ...
 *   · l'outil reste entièrement utilisable SANS réseau, par saisie manuelle du
 *     profil. Un outil qui exigerait le réseau pour fonctionner ferait dépendre
 *     une démonstration de la disponibilité d'un tiers.
 *
...
export async function profilDepuisIgn(
  ...
  for (let i = 0; i < points.length; i += POINTS_PAR_REQUETE) {
    ...
    try {
      ...
      throw new AltimetrieError(
        ...
        + 'Le profil peut être saisi à la main ci-dessous — l’outil ne dépend pas du réseau.',
      );
    }
    if (!reponse.ok) {
      throw new AltimetrieError(
        `Le service altimétrique de l’IGN a répondu ${reponse.status}. Le profil peut `
        + 'être saisi à la main ci-dessous.',
      );
    }
  ...
  if (couples.length < 2) {
    throw new AltimetrieError(
      ...
      + 'pas dire que le terrain est plat. Saisissez le profil à la main.',
    );
  }
