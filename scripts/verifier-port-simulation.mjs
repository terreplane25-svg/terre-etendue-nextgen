 ...
 */
function analyse(fractionVisible, obstacle, reliefEvalue) {
  const o = obstacle
    ? { distanceM: 1000, altitudeTerrainM: 50, altitudeViseeM: 40, manqueM: 10 }
    : null;
  return {
    modele: 'sphérique',
    rayonEffectifM: 7_000_000,
    hauteurOccultéeCourbureM: 0,
    fractionVisibleCourbure: fractionVisible,
    reliefEvalue,
    motifReliefNonEvalue: reliefEvalue ? null : 'non évalué',
    obstacles: o ? [o] : [],
    obstacleLePlusGenant: o,
    margeMinimaleM: o ? -10 : 5,
    distanceMargeMinimaleM: 1000,
  };
}
...
try {
  ...
  for (const cas of v.cas) {
    const a = analyse(cas.fraction_visible_borne_1, cas.obstacle, cas.relief_evalue);
    const b = analyse(cas.fraction_visible_borne_2, cas.obstacle, cas.relief_evalue);
    ...
    comparer(cas.nom, 'fraction cachée min', attendu.fraction_cachee_min, obtenu.fractionCacheeMin);
    comparer(cas.nom, 'fraction cachée max', attendu.fraction_cachee_max, obtenu.fractionCacheeMax);
    comparer(cas.nom, 'hauteur cachée min', attendu.hauteur_cachee_min_m, obtenu.hauteurCacheeMinM);
    comparer(cas.nom, 'hauteur cachée max', attendu.hauteur_cachee_max_m, obtenu.hauteurCacheeMaxM);
