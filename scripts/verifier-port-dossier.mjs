...
try {
  const sources = ['dossier.ts', 'conteneurs.ts', 'document.ts', 'isobmff.ts',
    'makernotes.ts', 'noyau.ts', 'provenance.ts', 'quantification.ts',
    'resolution.ts', 'telemetrie.ts'].map((f) => join(DIR, f));
  ...
  const modules = ['conteneurs', 'document', 'isobmff', 'makernotes', 'noyau',
    'provenance', 'quantification', 'resolution', 'telemetrie', 'dossier'];
  let total = 0;
