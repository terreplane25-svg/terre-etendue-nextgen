...
.n4{background:var(--bg);color:var(--t1);padding:0}
.n4-h{padding:5rem 2rem 3.5rem;text-align:center;position:relative;overflow:hidden}
.n4-h::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse 70% 50% at 50% 30%,rgba(212,175,55,.04),transparent 70%);pointer-events:none}
.n4-tag{display:inline-block;font-family:var(--f2);font-size:.62rem;font-weight:600;letter-spacing:.18em;text-transform:uppercase;color:var(--gold);background:var(--gold-g);border:1px solid rgba(212,175,55,.2);padding:.3em 1em;border-radius:4px;margin-bottom:1.5rem}
.n4-title{font-family:var(--f1);font-size:clamp(1.8rem,4.5vw,2.6rem);font-weight:700;color:var(--gold);line-height:1.15;margin:0 0 1rem;max-width:800px;margin-left:auto;margin-right:auto}
.n4-sub{font-family:var(--f3);font-size:1.02rem;line-height:1.7;color:var(--t2);max-width:650px;margin:0 auto 2rem}
.n4-meta{max-width:800px;margin:0 auto;padding:1.2em 0;border-top:1px solid var(--bdr);border-bottom:1px solid var(--bdr);display:flex;flex-wrap:wrap;gap:.5em 2em;font-family:var(--f2);font-size:.75rem;color:var(--t3)}
.n4-meta dt{font-weight:600;text-transform:uppercase;letter-spacing:.08em;font-size:.65rem}
.n4-meta dd{margin:0 0 .6em;color:var(--t2)}
.n4-lay{display:flex;gap:2.5rem;max-width:1200px;margin:0 auto;padding:2.5rem 2rem 5rem;align-items:flex-start}
.n4-nav{flex:0 0 255px;position:sticky;top:80px;max-height:calc(100vh - 100px);overflow-y:auto;padding:1.5rem;background:var(--bg2);border:1px solid var(--bdr);border-radius:10px}
.n4-nav::-webkit-scrollbar{width:3px}
.n4-nav::-webkit-scrollbar-thumb{background:var(--bdr);border-radius:3px}
.n4-nav__t{font-family:var(--f2);font-size:.65rem;font-weight:600;letter-spacing:.15em;text-transform:uppercase;color:var(--gold);margin:0 0 1rem;padding-bottom:.6rem;border-bottom:1px solid var(--bdr)}
.n4-nav ul{list-style:none;padding:0;margin:0}
.n4-nav li{margin-bottom:.35rem}
.n4-nav a{display:block;font-family:var(--f2);font-size:.76rem;color:var(--t3);text-decoration:none;padding:.4em .8em;border-radius:5px;border-left:2px solid transparent;transition:all .25s ease;line-height:1.35}
.n4-nav a:hover{color:var(--t1);background:rgba(212,175,55,.05);border-left-color:var(--gold)}
.n4-nav a.active{color:var(--gold);background:var(--gold-g);border-left-color:var(--gold);font-weight:500}
.n4-nav .num{font-size:.6rem;font-weight:600;color:var(--gold-s);margin-right:.4em;opacity:.7}
.n4-nav .sub{padding:0 0 0 1.2em;margin:.15rem 0 .2rem}
.n4-nav .sub a{font-size:.7rem;padding:.3em .8em}
.n4-nav__bk{display:block;margin-top:1.2rem;padding:.6em 1em;background:var(--gold);color:var(--bg);font-family:var(--f2);font-size:.72rem;font-weight:600;letter-spacing:.05em;text-transform:uppercase;text-decoration:none;text-align:center;border-radius:5px;transition:background .3s ease}
.n4-nav__bk:hover{background:#D4A017}
.n4-b{flex:1;min-width:0;max-width:800px;font-family:var(--f3);font-size:clamp(1rem,1.8vw,1.08rem);line-height:1.75;color:var(--t1)}
.n4-b p{margin-bottom:1.4em;text-align:justify;hyphens:auto}
.n4-b a{color:var(--gold);text-decoration:underline;text-underline-offset:3px;text-decoration-color:var(--gold-s)}
.n4-b h2{font-family:var(--f1);font-size:clamp(1.4rem,3vw,1.85rem);font-weight:700;color:var(--t1);margin:3em 0 .8em;padding-bottom:.4em;border-bottom:1px solid var(--bdr);line-height:1.25;scroll-margin-top:90px}
.n4-b h3{font-family:var(--f2);font-size:clamp(1rem,1.8vw,1.15rem);font-weight:600;color:var(--cobalt-l);text-transform:uppercase;letter-spacing:.06em;margin:2em 0 .6em;line-height:1.35;scroll-margin-top:90px}
...
.n4-refs{margin-top:3em;padding-top:2em;border-top:1px solid var(--bdr)}
.n4-refs h2{border-bottom:none;padding-bottom:0;font-size:1.3rem}
.n4-refs ol{padding-left:1.5em;font-family:var(--f2);font-size:.82rem;color:var(--t2);line-height:1.7}
.n4-refs li{margin-bottom:.6em}
@media(max-width:900px){.n4-lay{flex-direction:column;padding:1.5rem}.n4-nav{position:static;flex:none;width:100%;max-height:none;margin-bottom:2rem}.n4-h{padding:3.5rem 1.5rem 2.5rem}}


 Le Nexus — NXS-2026-004
...
# La gravité : 70 théories et aucune certitude
 ...
 DateAvril 2026 — v1.0
 Lecture~30 min
 DomaineCritique scientifique · Épistémologie
...
## Sommaire
 ...
 [01 Introduction](#g-intro)

 - [02 Fait vs interprétation](#g-fait)

 - [03 Newton et Einstein](#g-newton)
 [La critique de Mach](#g-mach)


 
 - [04 La constante G instable](#g-constante)

 - [05 Anomalies empiriques](#g-anomalies)
 [Pioneer](#g-pioneer)
- [Flyby](#g-flyby)
- [Allais](#g-allais)
 ...
 - [06 Univers à 95% hypothétique](#g-95)

 - [07 L'alternative : densité et Archimède](#g-densite)

 - [08 La cosmologie plasma](#g-alfven)
...
PériodeÉvénementCe qui change

1687–1727Newton vivantAgnosticisme méthodologique : « la cause m'est inconnue, je ne forge pas d'hypothèse »
~1730–1750Premiers newtoniens (Samuel Clarke et al.)Tentent de fournir les fondements métaphysiques que Newton avait **intentionnellement** refusés
XVIIIᵉ–XIXᵉ s.Auteurs de manuelsSimplifient la théorie newtonienne pour l'enseignement. Le refus de spéculer est remplacé par le récit d'une « force universelle découverte »
~1860+Maxwell et la physique classiqueL'identification de la gravité avec une force physique littérale devient standard, bien qu'elle **contredise la position explicite de Newton**


**Ce que les académiques confirment :** I. Bernard Cohen (Harvard) : « Newton n'a pas prétendu que la gravité était une force réelle dans la matière. » Andrew Janiak (Cambridge, 2008) : « Le traitement de la gravité par Newton est agnostique : il nie que la gravité soit un pouvoir réel inhérent aux corps. » E.J. Aiton (*Isis*, 1969) : « Pour Newton, la force gravitationnelle était un *effet*, pas une *cause*. » Richard Westfall (Cambridge, 1980) : « Newton a fermement refusé d'attribuer la gravité à une force inhérente à la matière. »
...
Composition du modèle standard (Λ-CDM)
...
**Instabilité structurelle :** Même les entités hypothétiques postulées pour sauver le modèle voient leurs propriétés fondamentales remises en question. Il ne s'agit plus d'affiner un modèle solide — il s'agit de réviser ses hypothèses de base.
...
La densité en action : exemples concrets
...
**L'ironie historique :** L'homme dont la théorie est censée prédire les trous noirs a lui-même démontré qu'ils ne peuvent pas exister. Cette démonstration, publiée dans une revue de premier plan, est citée dans les bases académiques mais systématiquement ignorée dans l'enseignement. Le physicien Eric J. Lerner documente comment la communauté a contourné la démonstration d'Einstein en invoquant des conditions « dynamiques » non couvertes par son article — un raisonnement circulaire.
...
InstitutionnelMécanisme de filtrage documenté (Smolin, Loeb)
...
**Ce que cela signifie :** Quand on dit « cette galaxie est à 13 milliards d'années-lumière », on *infère* une distance à partir d'un redshift, calibré par des supernovae, calibrées par des céphéides, calibrées par une parallaxe qui suppose le mouvement de la Terre. Ce n'est pas une mesure — c'est une cascade d'hypothèses dépendantes d'un modèle. (Voir aussi [L1 — Le MGPP](/le-modele-geocentrique-a-plans-paralleles/) pour un cadre alternatif d'interprétation des distances.)

### Halton Arp et le redshift intrinsèque

La relation redshift = distance est la pierre angulaire de la cosmologie moderne. Si le redshift n'est pas proportionnel à la distance, l'expansion de l'univers, le Big Bang et toutes les distances extragalactiques s'effondrent. Or **Halton Arp** (1927-2013), assistant de Hubble lui-même et l'un des astronomes observateurs les plus prolifiques du XXᵉ siècle, a documenté des dizaines de cas où des objets physiquement liés ont des redshifts radicalement différents.


L'exemple le plus célèbre : la galaxie NGC 4319 et le quasar Markarian 205 sont reliés par un **pont de matière lumineuse** visible sur les photographies. Ils sont donc à la même distance de nous. Or NGC 4319 a un redshift de z = 0,006 et Markarian 205 a un redshift de z = 0,070 — soit **12 fois plus**. Si le redshift mesurait la distance, le quasar serait 12 fois plus loin que la galaxie à laquelle il est physiquement connecté.


Arp a catalogué plus de 200 cas similaires dans son *Atlas of Peculiar Galaxies* (1966) et *Quasars, Redshifts and Controversies* (1987). Sa récompense : le temps d'observation au télescope lui a été retiré. Il a dû quitter les États-Unis pour poursuivre ses recherches au Max-Planck-Institut en Allemagne.


**L'implication :** Si le redshift contient une composante *intrinsèque* (liée aux propriétés de l'objet et non à sa distance), alors les distances extragalactiques calculées par redshift sont systématiquement fausses. L'univers observable pourrait être beaucoup plus petit que ce que le modèle standard prétend. Le besoin d'énergie noire et de matière noire — inventées pour réconcilier les distances avec les luminosités observées — pourrait simplement disparaître.
