...
────────────────────────────────────────────────────────
Le modèle de terrain avait été retiré, puis rétabli. Il est de nouveau relevé —
à la demande, jamais automatiquement — et rendu comme une INFORMATION : le
nombre de reliefs qui coupent la visée, et de combien chacun la dépasse.

Ce qu'il ne fait pas, et c'est la moitié importante : il n'entre pas dans le
verdict. La hauteur masquée, la fraction et le caractère discriminant répondent
à « que prédit chaque modèle », pas à « que voit-on depuis ce poste ». Les
fondre sous un seul « invisible » perdrait ce qui distingue les deux causes —
une cible peut être entièrement au-dessus de l'horizon géométrique ET
entièrement cachée par une colline.

Et un relief NON RELEVÉ n'est jamais présenté comme une absence d'obstacle.
...
def pas_echantillonnage_m(distance_m: float) -> float:
    ...
    Le pas retenu est rendu au visiteur, parce qu'il décide de ce qui peut
    passer inaperçu entre deux mesures.
    """
    if distance_m <= 0:
...
#:
#: Ce motif accompagne TOUT résultat de relief. Un modèle numérique de terrain
#: décrit le sol, à une date et à une résolution données : il ignore ce qui
#: bouche le plus souvent une visée réelle — une haie, un cargo, un bâtiment
#: récent. Le relief relevé réduit l'incertitude, il ne la supprime pas, et le
#: contrôle final reste celui de l'analyste sur l'image.
MOTIF_RELIEF_RELEVE = (
