...
#
# Identique à celui du premier cas d'étude (cas-cordouan/case_data.py) — repris
# ici pour que ce second dossier reste autonome et rejouable sans dépendre du
# premier. Le Tableau 10 (§12.3, visee_optique.geodesy) exige "un algorithme
# publié et vérifié" pour la distance et l'azimut géodésiques — jamais déduits
# de la photographie.
...
def vincenty_direct(lat1_deg, lon1_deg, azimut1_deg, distance_m, a, f):
    """Point géodésique atteint depuis (lat1, lon1) en suivant azimut1 sur
    distance_m mètres (Vincenty, 1975, formule directe). Retourne (lat_deg,
    lon_deg). Utilisé uniquement pour interpoler des points le long de la
    route Chassiron→Cordouan (vérification du profil intermédiaire,
    Tableau 10, §12.3) — jamais pour la distance/l'azimut eux-mêmes, qui
    restent calculés par la formule inverse ci-dessus.
    """
...
def calculer_verification_profil(chassiron_lat, chassiron_lon, cordouan_lat, cordouan_lon, a, f):
    ...
    """
    distance_totale, azimut, _ = vincenty_inverse(chassiron_lat, chassiron_lon, cordouan_lat, cordouan_lon, a, f)
